...
class TestDualFormatDeserialization:
    """Tests for backward-compatible deserialization of nested data.M format."""

    @pytest.mark.asyncio
    async def test_entity_nested_format(self, repo):
        """Entity with nested data.M should deserialize correctly."""
        from zae_limiter import schema

        client = await repo._get_client()
        # Write entity in old nested format
        await client.put_item(
            TableName=repo.table_name,
            Item={
                "PK": {"S": schema.pk_entity("legacy-entity")},
                "SK": {"S": schema.sk_meta()},
                "entity_id": {"S": "legacy-entity"},
                "data": {
                    "M": {
                        "name": {"S": "Legacy"},
                        "parent_id": {"NULL": True},
                        "metadata": {"M": {"tier": {"S": "free"}}},
        ...
        entity = await repo.get_entity("legacy-entity")
        assert entity is not None
        assert entity.name == "Legacy"
        assert entity.parent_id is None
        assert entity.metadata == {"tier": "free"}

    @pytest.mark.asyncio
    async def test_entity_flat_format(self, repo):
        """Entity with flat format should deserialize correctly."""
        from zae_limiter import schema
        ...
        await client.put_item(
            TableName=repo.table_name,
            Item={
                "PK": {"S": schema.pk_entity("flat-entity")},
                "SK": {"S": schema.sk_meta()},
                "entity_id": {"S": "flat-entity"},
                "name": {"S": "Flat"},
                "parent_id": {"S": "parent-1"},
                "metadata": {"M": {"tier": {"S": "premium"}}},
                "created_at": {"S": "2024-06-01T00:00:00Z"},
            },
        )

        entity = await repo.get_entity("flat-entity")
        assert entity is not None
        assert entity.name == "Flat"
        assert entity.parent_id == "parent-1"
        assert entity.metadata == {"tier": "premium"}

    @pytest.mark.asyncio
    async def test_bucket_nested_format(self, repo):
        """Bucket with nested data.M should deserialize correctly."""
        from zae_limiter import schema
        ...
        await client.put_item(
            TableName=repo.table_name,
            Item={
                "PK": {"S": schema.pk_entity("entity-1")},
                "SK": {"S": schema.sk_bucket("gpt-4", "rpm")},
                "entity_id": {"S": "entity-1"},
                "data": {
                    "M": {
                        "resource": {"S": "gpt-4"},
                        "limit_name": {"S": "rpm"},
                        "tokens_milli": {"N": "100000"},
                        "last_refill_ms": {"N": "1704067200000"},
                        "capacity_milli": {"N": "100000"},
                        "burst_milli": {"N": "100000"},
                        "refill_amount_milli": {"N": "100000"},
                        "refill_period_ms": {"N": "60000"},
                    }
                },
                "total_consumed_milli": {"N": "5000000"},
                "GSI2PK": {"S": schema.gsi2_pk_resource("gpt-4")},
                "GSI2SK": {"S": schema.gsi2_sk_bucket("entity-1", "rpm")},
            },
        )

        bucket = await repo.get_bucket("entity-1", "gpt-4", "rpm")
        assert bucket is not None
        assert bucket.resource == "gpt-4"
        assert bucket.limit_name == "rpm"
        assert bucket.tokens_milli == 100_000
        assert bucket.capacity_milli == 100_000
        assert bucket.total_consumed_milli == 5_000_000

    @pytest.mark.asyncio
    async def test_audit_nested_format(self, repo):
        """Audit event with nested data.M should deserialize correctly."""
        from zae_limiter import schema
        ...
        await client.put_item(
            TableName=repo.table_name,
            Item={
                "PK": {"S": schema.pk_audit("entity-1")},
                "SK": {"S": schema.sk_audit("01HQ0000000000")},
                "entity_id": {"S": "entity-1"},
                "data": {
                    "M": {
                        "event_id": {"S": "01HQ0000000000"},
                        "entity_id": {"S": "entity-1"},
                        "timestamp": {"S": "2024-01-01T00:00:00Z"},
                        "action": {"S": "entity_created"},
                        "principal": {"S": "admin"},
                        "resource": {"NULL": True},
                        "details": {"M": {"name": {"S": "Test"}}},
                    }
                },
                "ttl": {"N": "9999999999"},
            },
        ...
        events = await repo.get_audit_events("entity-1")
        assert len(events) == 1
        assert events[0].event_id == "01HQ0000000000"
        assert events[0].action == "entity_created"
        assert events[0].principal == "admin"
        assert events[0].details == {"name": "Test"}

    @pytest.mark.asyncio
    async def test_version_nested_format(self, repo):
        """Version record with nested data.M should deserialize correctly."""
        from zae_limiter import schema
        ...
        assert version is not None
        assert version["schema_version"] == "1.0.0"
        assert version["lambda_version"] == "0.5.0"
        assert version["updated_by"] == "test"

    @pytest.mark.asyncio
    async def test_limits_nested_format(self, repo):
        """Entity limits with nested data.M should deserialize correctly."""
        from zae_limiter import schema
        ...
        await client.put_item(
            ...
            Item={
                "PK": {"S": schema.pk_entity("entity-1")},
                "SK": {"S": schema.sk_limit("gpt-4", "rpm")},
                "entity_id": {"S": "entity-1"},
                "data": {
                    "M": {
                        "resource": {"S": "gpt-4"},
        ...
        limits = await repo.get_limits("entity-1", resource="gpt-4")
        assert len(limits) == 1
