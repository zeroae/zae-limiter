...
class TestDurationWindowStamp:
    ...
    @pytest.mark.asyncio
    async def test_normal_write_stamps_a_changed_length_alone(self, repo):
...
class TestPropagateWindowStart:
    ...
    W_MS = 5 * 3_600_000
    OLD = 1_000
    NEW = OLD + W_MS  # the old window's end: half-open, so it has elapsed
    ...
    def _windows(self, ws, rsa=18_000):
        return {"session": (ws, rsa)}

    @pytest.mark.asyncio
    async def test_writes_every_other_shard(self, repo):
        await self._create_shards(repo, "e1", count=4, ws=self.OLD)
        ...
        for shard in (0, 1, 3):
            assert await self._stored_ws(repo, "e1", "session", shard) == self.NEW
        # The writer's own shard is stamped by its transaction, not by this.
        assert await self._stored_ws(repo, "e1", "session", 2) == self.OLD

    @pytest.mark.asyncio
    async def test_the_boundary_is_the_half_open_window_end(self, repo):
    ...
    async def test_a_concurrent_opener_does_not_move_the_other_openers_shard(self, repo):
        ...
        await self._create_shards(repo, "e1", count=2, ws=self.NEW)
        written = await repo._propagate_window_start(
    ...
    @pytest.mark.asyncio
    async def test_lands_rsa_beside_ws_on_a_sibling_that_had_none(self, repo):
    ...
    async def test_is_a_noop_with_no_window(self, repo):
        with patch.object(repo, "_get_client") as mock_get_client:
            assert (
                await repo._propagate_window_start(
                    "e1", "gpt-4", shard_id=0, shard_count=4, windows={}
                )
                == 0
            )
            mock_get_client.assert_not_called()
    ...
    async def test_writes_once_per_sibling_and_limit(self, repo):
        ...
        await self._create_shards(repo, "e1", count=3, ws=self.OLD, limits=[self.SESSION, daily])
        written = await repo._propagate_window_start(
            "e1",
            "gpt-4",
            shard_id=0,
            shard_count=3,
            windows={"session": (self.NEW, 18_000), "daily": (self.NEW, 86_400)},
        )
        assert written == 2, "session on shards 1 and 2; the daily window has not ended"
        for shard in (1, 2):
            assert await self._stored_ws(repo, "e1", "session", shard) == self.NEW
            assert await self._stored_ws(repo, "e1", "daily", shard) == self.OLD
    ...
    async def test_does_not_create_a_missing_shard(self, repo):
        ...
        `ws`, `rsa` and `vu`."""
        await self._create_shards(repo, "e1", count=1, ws=self.OLD)
        written = await repo._propagate_window_start(
            "e1", "gpt-4", shard_id=0, shard_count=2, windows=self._windows(self.NEW)
        )
        assert written == 0
    ...
    @pytest.mark.asyncio
    async def test_a_failed_sibling_write_does_not_abandon_the_rest(self, repo, caplog):
    ...
    @pytest.mark.asyncio
    async def test_a_conditional_miss_is_not_logged(self, repo, caplog):
        await self._create_shards(repo, "e1", count=2, ws=self.NEW)
        with caplog.at_level("WARNING", logger="zae_limiter"):
            written = await repo._propagate_window_start(
                "e1", "gpt-4", shard_id=0, shard_count=2, windows=self._windows(self.NEW)
    ...
    @pytest.mark.asyncio
    async def test_cancellation_still_propagates(self, repo):
...
class TestStaleLimitAliasesAreExpressionSafe:
    ...
    async def test_hyphenated_and_dotted_stale_names_build_legal_aliases(self, repo):
        ...
        # 13 for `sched`, surface Task 4 for `rsched`) and its `rsa` (plan
        # Task 9, ADR-139): `rpm` has no window, so its `rsa` is REMOVEd
        # exactly as an unscheduled limit's `sched`/`rsched` overrides are —
        # absence means "no window", full stop, with no default to inherit.
...
class TestDurationWindowParamSync:
    ...
    async def test_the_fan_out_stamps_rsa_on_an_existing_bucket(self, repo):
        window = Limit.quota("rpd", 10_000, reset_after=timedelta(hours=5))
        await repo.create_entity("dw-fan-1")
        await repo.set_limits("dw-fan-1", [Limit.per_day("rpd", 10_000)], resource="gpt-4")
        now_ms = int(time.time() * 1000)
        states = [BucketState.from_limit("dw-fan-1", "gpt-4", Limit.per_day("rpd", 10_000), now_ms)]
        await repo.transact_write(
            [repo.build_composite_create("dw-fan-1", "gpt-4", states, now_ms)]
        )

        await repo.set_limits("dw-fan-1", [window], resource="gpt-4")

        item = await self._raw_bucket(repo, "dw-fan-1", "gpt-4")
        assert item[bucket_attr("rpd", "rsa")]["N"] == "18000"
        assert bucket_attr("rpd", "ws") not in item

    async def test_removing_a_window_removes_the_stamp(self, repo):
        window = Limit.quota("rpd", 10_000, reset_after=timedelta(hours=5))
        await repo.create_entity("dw-fan-2")
        await repo.set_limits("dw-fan-2", [window], resource="gpt-4")
        now_ms = int(time.time() * 1000)
        states = [BucketState.from_limit("dw-fan-2", "gpt-4", window, now_ms)]
        await repo.transact_write(
            [repo.build_composite_create("dw-fan-2", "gpt-4", states, now_ms)]
        )
        item = await self._raw_bucket(repo, "dw-fan-2", "gpt-4")
        assert item[bucket_attr("rpd", "rsa")]["N"] == "18000"

        await repo.set_limits("dw-fan-2", [Limit.per_day("rpd", 10_000)], resource="gpt-4")
