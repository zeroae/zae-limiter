...
# never the limit name (#634), alphanumeric only, and disjoint from every other
# family on the same expression (`#bt{i}`, `#ws{i}`, `#rsa{i}`, `#wl{i}`, the
# fixed `#rf` / `#ttl` / `#vu`).
_SEED_TOKEN = {
...
class Repository:
    ...
    def build_composite_create(
        ...
        for state in states:
            # One helper for the per-limit attributes, shared with the seed a
            # limit missing from an existing item gets on the normal write
            # path (#633), so the two cannot drift. `wcu` is auto-injected
            # above and never reaches this loop, so it can never carry a
            # window.
    ...
    def build_composite_normal(
        ...
        if seeded_tz is not None:
            ...
            # single zone, so the seed's zone is the item's.
    ...
    def build_composite_retry(
        ...
        for j, (name, state) in enumerate(sorted((seeds or {}).items())):
            ...
            # The starting share before this acquire's consumption, which the
            # expression subtracts itself so that a limit seeded by another
            # writer in the meantime is debited rather than overwritten.
    ...
    def _deserialize_composite_bucket(self, item: dict[str, Any]) -> list[BucketState]:
        ...
        # Discover limit names by scanning for b_{name}_tk attributes. A `tk`
        # with no `cp` beside it is not a limit: an older client's slow path
        # created exactly that (`ADD b_{name}_tk 0`) for a limit configured
        # after the item existed (#633), and zero-filling the missing
        # parameters turned every later read into a division by zero. Skipped,
        # the limit reads as missing, the slow path seeds it, and the seed's
        # `attribute_not_exists(cp)` guard overwrites the stray balance.
