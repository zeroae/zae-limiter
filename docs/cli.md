...
| `--create-iam-roles` | Create App/Admin/ReadOnly IAM roles | `false` |
| `--iam/--no-iam` | Create IAM resources (policies, roles) | `true` |
| `--aggregator-role-arn` | Use existing IAM role ARN for Lambda aggregator | None |
...
# Restricted IAM environment (no IAM permissions)
zae-limiter deploy \
    --name myapp \
    --region us-east-1 \
    --no-iam

# Use pre-existing Lambda role
zae-limiter deploy \
    --name myapp \
    --region us-east-1 \
    --aggregator-role-arn arn:aws:iam::123456789012:role/MyLambdaRole

# Both: no IAM created, uses external role
zae-limiter deploy \
    --name myapp \
    --region us-east-1 \
    --no-iam \
