...
2. Analyze batch sizes:
   ```sql
   fields @timestamp, @message
   | filter @message like /record_count/
   | parse @message /record_count":(?<records>\d+)/
   | stats avg(records), max(records) by bin(1h)
   ```
...
**Symptoms:** Stream iterator age alarm triggered, growing lag
...
- Fix Lambda errors (check DLQ for failed batches)
...
1. Inspect DLQ messages:
   ...
   aws sqs receive-message \
       --queue-url https://sqs.us-east-1.amazonaws.com/123456789012/ZAEL-limiter-aggregator-dlq \
       --max-number-of-messages 10
   ```

2. Check Lambda logs around failure time
3. Verify DynamoDB table schema matches expected format

**Solutions:**

- Fix data format issues in stream records
- Reprocess DLQ messages after fixing root cause
- Increase Lambda timeout if processing takes too long

### DynamoDB Throttling

**Symptoms:** Read/write throttle alarms triggered

**Diagnostic steps:**

1. Check capacity utilization in CloudWatch
2. Identify hot partitions using CloudWatch Contributor Insights
3. Review access patterns for inefficiencies

**Solutions:**

- Switch to provisioned capacity with auto-scaling
- Distribute load across more partition keys
- Optimize access patterns (reduce cascade operations)

!!! tip "Capacity Planning"
    For detailed capacity calculations and cost optimization, see the [Performance Tuning Guide](performance.md#1-dynamodb-capacity-planning).
