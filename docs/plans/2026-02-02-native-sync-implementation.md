...
**Architecture:** Write async code as source of truth → AST transformer generates sync equivalents → Generated code committed to git → CI verifies sync code is up-to-date.
...
- Missing import rewrites → add to `IMPORT_PATH_REWRITES`
- Incorrect class references → add to `CLASS_RENAMES`
- asyncio imports not removed → add removal logic
...
1. ✅ AST transformer generates native sync code from async source
2. ✅ Pre-commit hook verifies generated code is up-to-date
3. ✅ Hatch build hook runs generation before packaging
4. ✅ SyncRateLimiter is now native boto3 (no event loop wrapper)
5. ✅ Tests pass for both async and sync implementations
6. ✅ Gevent/Eventlet compatibility verified
7. ✅ CI verifies generated code matches source
8. ✅ ADR documents the architecture decision
9. ✅ CLAUDE.md updated with generation workflow
