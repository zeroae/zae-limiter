...
**Entity metadata cache (issue #318):** `Repository._entity_cache` stores `{entity_id: (cascade, parent_id)}` as immutable metadata with no TTL. After the first acquire populates the cache (from `ALL_NEW` on speculative success or from the entity META record on slow path), subsequent cascade acquires use `_try_parallel_speculative()` which issues child and parent speculative writes concurrently via `asyncio.gather`. This reduces cascade latency from 2 sequential round trips to 1 parallel round trip while maintaining the same WCU cost. In sync mode, `asyncio.gather` is transformed to a sequential tuple by the code generator.
