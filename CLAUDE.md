...
- **The parent shards independently of the child (#474):** its shard is drawn from the **parent's own** cached `shard_count` (`select_shard(parent_id, resource)`), once per acquire, and every downstream path reuses that `parent_result.shard_id` — compensation, `_try_parent_only_acquire`, and `_do_acquire`'s parent branch. The parent's `shard_count` is learned from its own ALL_NEW image, and parent `wcu` exhaustion doubles the **parent's** count (`_shard_after_wcu_exhaustion`, shared with the child path). Hardcoding the parent to shard 0 left the one partition every cascading child writes to as an unmitigated hot partition
...
| Limit-change sync, per shard (#468) | `SET cp/ra/rp (+ ttl) REMOVE stale` | `attribute_exists(PK)` | No |
