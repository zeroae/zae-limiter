...
| Client shard create (ADR-133) | `Put` full item, `tk = effective cp // shard_count`, `wcu` undivided, `sched`/`rsched`/`sched_tz`/`vu` when scheduled | `attribute_not_exists(PK)` | Sets `rf = now` |
...
| Limit-change sync, per shard (#468), per resource under `_default_` (#487) | `SET cp/ra/rp, sched/rsched/sched_tz, vu = 0 (+ ttl) REMOVE stale, superseded per-limit sched/rsched` | `attribute_exists(PK)` | No |
