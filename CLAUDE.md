...
| Normal path (retry) | `ADD tk -consumed`; a still-missing limit: `SET tk = if_not_exists(tk, :share) − :c`, params `if_not_exists` (#633). `ReturnValuesOnConditionCheckFailure=ALL_OLD` | `tk >= consumed` (a seed: `attribute_not_exists(tk) OR tk >= :c`) | No (skips refill) |
