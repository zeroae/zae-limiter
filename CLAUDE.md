...
- **A rejection from the consumption-only retry is reported from the item, not from memory (#633).** That retry runs because the rf lock was lost — the in-memory state is exactly what just proved stale — so `build_composite_retry` asks for `ReturnValuesOnConditionCheckFailure=ALL_OLD` and `lease._build_retry_failure_statuses` builds each declared status from the failed item's real `tk` and `rf` (`exceeded` = the condition that failed, `tk < consumed`; `retry_after_seconds` = `try_consume` on the real balance, floored at 1 ms for an exceeded status so it is never a literal `0.0`). From memory the deficit was always 0, so every such rejection quoted `retry_after_seconds = 0.0` — a hot retry loop driven by the 429. Free: the image rides on the failure response. Falls back to the in-memory statuses when no image matches what failed
...
- **A limit configured after its bucket item existed is seeded, per limit (#633).** Resource- and system-level changes never touch buckets (#271/#296) and the entity-level sync writes `cp`/`ra`/`rp` but never `tk`, so the new limit is simply absent from the item — and every write guarded on it (`tk >= c` on a missing attribute is false) failed forever, with `retry_after_seconds = 0.0`. `_do_acquire` therefore decides newness **per limit**: `_is_new` stays the item-level create flag (whole-item `Put`), and a limit missing from an item that exists — "exists" read off every limit on it, `wcu` and no-longer-configured ones included — is flagged `_seed` and **SET in full on the same rf-locked write** by `build_composite_normal(seeds=...)`: `tk` at its per-shard, schedule-effective starting share less this acquire's consumption, `tc`, `cp`/`ra`/`rp`, and **explicit** `b_{n}_sched`/`b_{n}_rsched` (compact or `BUCKET_SCHED_NONE`, so the item default cannot leak onto it, #541). The per-limit attributes come from `Repository._limit_item_attrs`, shared with `build_composite_create`. Each seed is guarded by `attribute_not_exists(b_{n}_cp) OR attribute_not_exists(b_{n}_tk)`; the `rf` lock serialises seeders, the guard covers two landing in one millisecond (when `rf` does not move). The lock value is the item's `rf`, never the seed's synthetic `now` — with the new limit resolved first, `group_entries[0]` used to be the seed and the lock never matched, so refill was never credited again. The consumption-only retry seeds too (`if_not_exists`), since the lock can be lost to a writer that does not seed (the aggregator) — but only a limit it **debits** (`c > 0`) and only an **unscheduled** one: the retry stamps no `vu`, so a quota, session window or schedule seeded there could be spent by the fast path past its first boundary, and seeding a limit it does not debit would write `cp` beside a stray old-client `tk = 0`, making a repairable item read as spent. Anything else waits for the next rf-locked pass (at most one extra rejection). A **quota** seed on a sharded entity must not mint (#587): `Repository.reclaim_quota_seed` grants the full current share `C // S` unless some sibling **holds** more than that share, and then clamps every such sibling and seeds what the clamp took. Judged on the surplus held, never on `tc`: `tc` is cumulative across periods, so a sibling that consumed before a reset edge or session rollover read as over-granted forever and the shard was never seeded (review of #633). `S` is the largest of the cached count, the item's and every sibling's stored count (off the items already read). Why the full share is safe: each shard is granted at most one share per period, and shares at one count sum to `C`; only a grant taken at a **lower** count earlier in the period exceeds it, and while unspent that is visible as held surplus. Two further gaps are closed. **A transfer seed the pass does not write** (the acquire is rejected, or its rf-locked write loses the lock) is written by `Repository.persist_seed` at what the clamp took, with nothing consumed (`SET` under `attribute_not_exists(tk)`, `vu` only lowered; before the retry, so the retry can debit it) — otherwise the clamped surplus is destroyed and the next pass seeds in full. The clamp already writes on that path, so this completes the transfer rather than weakening write-on-enter further. The persist leaves `rf` alone, so it is **skipped** when the item's `rf` is behind the limit's current period — a calendar reset edge (`prev_reset_edge`) or a joined session window's start after the read `rf` — because the next slow pass would apply that edge and reset the shard to a full share on top of the persisted transfer the fast path had already spent (measured 1200+ against 1000); the unpersisted transfer's over-grant is inside the residual below. The seed on the rf-locked write has no such gap: it stamps `rf = max(now, stored rf, ws)` (`_monotonic_rf`), at or past any edge `now` has crossed. **A doubling racing the seed** — the aggregator's proactive doubling lands between the read and the rf-locked write without moving `rf`, and its Path 2 clone of the pre-seed image lacks the quota — is closed by pinning every quota seed's write on `attribute_not_exists(shard_count) OR shard_count <= :sized`; a lost pin falls to the retry (which never seeds a quota), costing one rejection. The refill clamp does **not** protect a quota here: it never drips, and the fast path's pure `ADD` spends the oversized share before any materialising pass trims it. Measured before the pin: 1500 admitted against 1000. **One residual remains, tracked separately:** a sibling granted a share at a lower count in this period that has **already spent** into its surplus — the spent part is gone from `tk` and indistinguishable from an earlier period's spend without a per-period grant record. Preconditions: that lower-count grant and a shard that lacks the quota after it, from either of two sources, neither needing a race: a client whose config cache has not yet seen the quota (within `config_cache_ttl` of configuring it) creating a shard across a doubling, or the aggregator's proactive doubling cloning new shards from an **unseeded shard 0**'s image, which carries and reclaims only the quotas shard 0 has, so a sibling seeded earlier keeps a surplus nobody reclaims (measured 1250 against 1000). Bound: at most the part of that sibling's surplus it had spent, `C · (1/S' − 1/S)` per such sibling, once, in the period of the doubling. A transfer can under-grant for the rest of a period, never beyond the next reset. A **session** quota seed on a sharded entity joins shard 0's live window (one strongly consistent `get_shard_window_starts` read) or opens and fans out its own, exactly like a created shard N>0. `_deserialize_composite_bucket` ignores a `b_{n}_tk` with no `b_{n}_cp` (what the pre-fix slow path created for an undeclared new limit), so such an item reads as missing the limit and self-repairs on the next slow pass; an entity-level item an old client seeded `tk = 0` *with* `cp` cannot be told from a spent balance and is left alone (one period of under-admission). Cost: the seed rides the slow pass the failed fast-path condition already routes to — same reads, same one write (larger by ~60–150 B, so 1 WCU unless the item crosses 1 KB) — once per (bucket, new limit); afterwards the fast path is 0 RCU + 1 WCU again. A quota seed on a sharded entity adds 1 GSI3 Query + 1 `BatchGetItem` (+ one clamp per surplus sibling) on each slow pass until the seed lands; a session-quota seed on a sharded entity adds 1 strongly consistent `GetItem`. Known cost wart: the fast path misclassifies a missing limit as `APP_LIMIT_EXHAUSTED`, so on a sharded entity the first acquire after the change can burn a failed probe of another shard (1 WCU) before the slow pass
...
- Issues `UpdateItem` with `ADD #rt{i} :rd{i}` (`#rt{i}` → `b_{limit}_tk`, one pair per refilled or reset limit) `SET rf = :new_rf` and condition `rf = :expected_rf`. The attribute is always aliased, never written inline: an inline `b_rpm.v2_tk` parses as a nested path (#634)
...
attribute name. `NAME_PATTERN` accepts `.` and `-`; a token may hold only letters, digits and `_`
(the AWS docs allow `_` in names but state only alphanumerics for values, so new tokens use
`[A-Za-z0-9]` only), and `.` in expression text is a document-path separator. Building tokens from
...
they cannot collide with the fixed `#wcu_tk` / `:neg_wcu` family on the same expression. `tests/unit/test_expression_tokens.py`
checks every write builder against the token grammar **and** that every token used is declared
and every token declared is used — DynamoDB rejects both an undeclared and an unused one. A new
bucket write belongs in that file.
